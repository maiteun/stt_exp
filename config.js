...
window.EXPERIMENT_CONFIG = {
  ...
  consent: {
    ...
    items: [
      ...
      "개인정보 보호: 모든 응답은 전체 참가자 데이터와 함께 묶여 집단 단위로만 분석됩니다. 개인의 점수나 응답이 따로 평가·공개되지 않으며, 영어 점수도 참가자 구성을 파악하기 위한 배경 변수로만 쓰입니다.",
  ...
  basicInfo: {
    title: "기본 정보",
    hint: "이름과 전화번호는 연구자 연락용으로 수집됩니다. 데이터베이스에는 익명 ID로만 저장되며, 분석 시 개인정보는 사용되지 않습니다. 영어 점수는 참가자 집단을 분석하기 위한 배경 변수일 뿐, 개인을 평가하는 데 쓰이지 않습니다.",
    fields: [
  ...
  settings: {
    ...
    shuffleOptions:   true   // 보기 순서를 참가자마다 무작위화 (채점은 원래 인덱스 기준 유지)
  }
};
